#pylint: disable=invalid-name, line-too-long, eval-used
...
HASH_ALGO = zlib.crc32 # crc32 or adler32
...
def code_path_1( block_size: int ):
    ...
    '''
    digest = DEFAULT_VALUE[HASH_ALGO]
    with TEST_FILE.open('rb') as f:
        while True:
            data = f.read(block_size)
            if not data:
                break
            digest = HASH_ALGO( data, digest ) # data, value
    return digest
...
def code_path_2( block_size: int ):
    ...
    with TEST_FILE.open('rb', buffering=0) as f:
        for n in iter(lambda : f.readinto(mv), 0):
            digest = HASH_ALGO( mv[:n], digest ) # data, value
    return digest
...
def test_blocks() -> None:
    ...
    sanity_check = {
        f"{test}-2^16": hex(eval(f"{test}( 2**16 )"))
